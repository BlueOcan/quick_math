...
class _LogicGameScreenState extends State<LogicGameScreen>
  ...
  // ── Questions ──────────────────────────────────────────────────
  late List<LogicQuestion> _questions;
  int _qIndex = 0;
  ...
  void initState() {
    ...
    LogicQuestionGenerator.resetSession();
    _questions = List.generate(
        widget.totalQuestions,
        (_) => LogicQuestionGenerator.generate(
  ...
  void _goToResults() async {
    ...
    Navigator.of(context).pushReplacement(PageRouteBuilder(
      pageBuilder: (_, anim, __) => LogicResultScreen(
        score: _score,
        correct: _correct,
...
class _OptionButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Color bgColor = surface, bdColor = border, textColor = tp;
    Color letterBg = color.withValues(alpha: 0.1), letterColor = color;
